...
export default function NotificationDropdown({ userId }: { userId?: string }) {
  ...
  useEffect(() => {
    const toItem = (n: DbNotification): NotificationItem => ({
      id: n.id,
      type: n.type,
      title: n.title,
      message: n.message,
  ...
  const handleOpenToggle = () => {
    ...
    if (willOpen) {
      ...
      if (userId) {
        supabaseDbService.markNotificationsRead(userId);
      }
    }
  };
  ...
  const markRead = (id: string) => {
    ...
    setNotifications(updated);
    supabaseDbService.markNotificationRead(id);
  };
                ...
                <li key={n.id} className={`p-2 rounded ${n.read ? 'bg-card' : 'bg-gradient-to-r from-white to-[#f0fdf4]'} border border-border`}> 
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center gap-2">
                        {!n.read && <button onClick={() => markRead(n.id)} title="Mark read" className="p-1 rounded hover:bg-accent"><Check className="w-4 h-4" /></button>}
                        <button onClick={() => deleteNotif(n.id)} title="Delete" className="p-1 rounded hover:bg-accent"><Trash2 className="w-4 h-4" /></button>
                      </div>
                      {n.path && <button onClick={() => onClickNotification(n)} className="text-xs text-muted-foreground hover:text-foreground">Open</button>}
                    </div>
                  </div>
                </li>
              ))}
